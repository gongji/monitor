		...
		public class ListViewItemCollection : IList, ICollection, IEnumerable
		{
			...
			private void RemoveInternal(ListViewItem item)
			{
				...
				if (selectedItemRemoved)
				{
					...
					{
						newSelectedItemIndex = Mathf.Min(Mathf.Max(selectedItemIndex, 0), this.items.Count - 1);
						this.owner.SelectedIndices.AddAndUpdateSelection(newSelectedItemIndex, raiseEvent: false);
					}
				}
